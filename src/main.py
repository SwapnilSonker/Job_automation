...
<head>
  <meta charset="utf-8">
  <style>
    body {{ font-family: 'Segoe UI', Arial, sans-serif; color: #333333; line-height: 1.6; background-color: #f0f2f5; margin: 0; padding: 20px; }}
    ...
    th {{ background-color: #f8fafc; color: #475569; font-size: 12px; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; }}
    tr:hover {{ background-color: #f8fafc; }}
    .email-link {{ font-weight: bold; color: #0073b1; text-decoration: none; }}
    .email-link:hover {{ text-decoration: underline; }}
    .jd-text {{ font-size: 13px; color: #475569; white-space: pre-line; }}
  ...
  <div class="container">
    ...
    <table>
      <thead>
        <tr>
          ...
          <th style="width: 70%;">Job Description Summary</th>
        </tr>
      </thead>
      <tbody>
        {rows_html}
    ...
    try:
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
            ...
            server.sendmail(GMAIL_ADDRESS, [e.strip() for e in recipient_email.split(",") if e.strip()], msg.as_string())
        log.info(f"[REPORT] Curated report successfully sent to {recipient_email}")
        return True
    except Exception as e:
        log.error(f"[REPORT] Failed to send curated report: {e}")
        return False
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "login":
        ...
        recipient = sys.argv[2] if len(sys.argv) > 2 else os.getenv("REPORT_EMAILS", os.getenv("NOTIFICATION_EMAIL", GMAIL_ADDRESS))
        send_curated_sheet_report(recipient)
    else:
